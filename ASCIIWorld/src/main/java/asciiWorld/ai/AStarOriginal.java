...
public class AStarOriginal {
	...
	public static List<Point> findPath(IAStarMap map, Point pathStart, Point pathEnd) {
		...
		*/
		...
		List<AStarNode> closedNodes = new ArrayList<AStarNode>(); // list of closed Nodes
		...
		while ((length = openNodes.size()) > 0) {
			...
			if (myNode.value == myPathEnd.value) { // is it the destination node?
				closedNodes.add(myNode);
				myPath = closedNodes.get(closedNodes.size() - 1);
				do {
				...
				for (int i = 0, j = myNeighbours.size(); i < j; i++) {
					...
					if (!visitedNodes[myPath.value]) {
						myPath.g = myNode.g + distance(distanceFn, myNeighbours.get(i), myNode); // estimated cost of this particular route so far
						myPath.f = myPath.g + distance(distanceFn, myNeighbours.get(i), myPathEnd); // estimated cost of entire guessed route to the destination
				...
				closedNodes.add(myNode); // remember this route as having no more untested options
			}
